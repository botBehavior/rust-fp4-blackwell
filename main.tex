...
\label{sec:related}
\textbf{Rust on GPUs.} Compiling Rust to GPUs is an active and consolidating space. \texttt{rust-gpu}~\citep{rustgpu} (a \texttt{rustc} backend emitting SPIR-V/Vulkan shaders) and the Rust-CUDA project's \texttt{rustc\_codegen\_nvvm}~\citep{rustcuda} (Rust$\rightarrow$NVVM$\rightarrow$PTX) are now both maintained by VectorWare~\citep{vectorware}, whose newer std-on-CUDA work runs general-purpose standard-library Rust on NVIDIA GPUs by mapping each \texttt{std::thread} to a GPU warp --- with warp-lockstep \emph{convergence}, for which Rust has no semantics, as the central open problem~\citep{kostrubiec2026}. Cross-platform kernel DSLs such as \texttt{CubeCL}~\citep{cubecl} compile a single \texttt{\#[cube]} Rust function to CUDA, ROCm, WGPU, SPIR-V, or Metal; host-side binding and launch crates such as \texttt{cudarc}~\citep{cudarc}, and re-targeting layers such as \texttt{ZLUDA}~\citep{zluda} (which runs CUDA binaries on non-NVIDIA hardware), round out the ecosystem. \texttt{cuda-oxide} is distinct from all of these: NVIDIA's own first-party experimental Rust$\rightarrow$PTX backend (Rust MIR $\rightarrow$ Pliron IR $\rightarrow$ LLVM $\rightarrow$ PTX), neither a shader path nor a std-on-GPU runtime. It is the toolchain whose consumer-Blackwell surface we measure, and to our knowledge none of these stacks has been driven end-to-end for FP4 LLM inference.
...
\bibitem{cudaoxide} NVIDIA Labs. \emph{cuda-oxide}. \url{https://github.com/NVlabs/cuda-oxide}.
\bibitem{llamacpp} G.~Gerganov and the llama.cpp contributors. \emph{llama.cpp}, 2023. \url{https://github.com/ggml-org/llama.cpp}.
\bibitem{tinyllama} P.~Zhang, G.~Zeng, T.~Wang, and W.~Lu. TinyLlama: An open-source small language model. \emph{arXiv:2401.02385}, 2024.
\bibitem{ocpmx} Open Compute Project. \emph{OCP Microscaling Formats (MX) Specification}, Version 1.0, 2023.
\bibitem{ptxisa} NVIDIA. \emph{Parallel Thread Execution ISA}. \url{https://docs.nvidia.com/cuda/parallel-thread-execution/index.html}.
\bibitem{rtx5070ti} NVIDIA. \emph{GeForce RTX 5070 Ti}, 2025. 16\,GB GDDR7, 256-bit, 896\,GB/s.
\bibitem{rustcuda} Rust GPU contributors. \emph{The Rust CUDA Project}. \url{https://github.com/Rust-GPU/Rust-CUDA}.
\bibitem{rustgpu} Rust GPU contributors. \emph{Rust GPU}. \url{https://github.com/Rust-GPU/rust-gpu}.
\bibitem{cudarc} C.~Lowman. \emph{cudarc: Safe Rust Wrappers for CUDA}. \url{https://github.com/coreylowman/cudarc}.
\bibitem{zluda} A.~Janik. \emph{ZLUDA: CUDA on Non-NVIDIA GPUs}. \url{https://github.com/vosen/ZLUDA}.
\bibitem{candle} Hugging Face. \emph{candle: Minimalist ML Framework for Rust}. \url{https://github.com/huggingface/candle}.
\bibitem{vectorware} VectorWare. Rust's standard library on the GPU, 2026. \url{https://www.vectorware.com/blog/rust-std-on-gpu/}.
\bibitem{kostrubiec2026} M.~Kostrubiec. Why Rust's semantics don't fit GPUs (yet). RustWeek 2026 talk. \url{https://www.youtube.com/watch?v=XnU9eBNTHG0}.
\bibitem{cubecl} Tracel AI. \emph{CubeCL: Multi-platform High-performance Compute Language Extension for Rust}. \url{https://github.com/tracel-ai/cubecl}.
\end{thebibliography}
\end{document}
